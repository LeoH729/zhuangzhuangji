...
Page({
  ...
  async onLoad(options) {
    ...
    preloadRewardAsset('boost').catch(() => {})
    ...
    if (id) {
      this.setData({ id, sourceZone, sourceGroup })
      this.fetchDetail(id)
    } else {
  ...
  reportFeatureDetailView(feature) {
    ...
    report('feature_detail_view', {
      ...
      template_version_id: feature.publishedVersionId || '',
      feature_name: feature.name || '',
      category_id: this.data.sourceGroup || feature.group || '',
      zone: this.data.sourceZone || feature.placements && feature.placements[0] && feature.placements[0].zone || '',
      source_page: 'template_detail'
    })
  },
  ...
  async handleBoostGeneration(taskId) {
    ...
    try {
      ...
      if (task && task.status === 'succeeded' && task.historyId) {
        ...
        wx.navigateTo({
          url: `/pages/result/result?id=${encodeURIComponent(task.historyId)}&featureId=${encodeURIComponent(task.featureId || this.data.id)}${this.data.sourceZone ? `&sourceZone=${this.data.sourceZone}` : ''}${this.data.sourceGroup ? `&sourceGroup=${encodeURIComponent(this.data.sourceGroup)}` : ''}`
        })
        return
      }
  ...
  goToAnalyzingWithTask(taskId, boosted) {
    ...
    wx.navigateTo({
      url: `/pages/analyzing/analyzing?featureId=${encodeURIComponent(this.data.id)}&taskId=${encodeURIComponent(taskId)}&boosted=${boosted ? '1' : '0'}${this.data.sourceZone ? `&sourceZone=${this.data.sourceZone}` : ''}${this.data.sourceGroup ? `&sourceGroup=${encodeURIComponent(this.data.sourceGroup)}` : ''}`
    })
  },
