...
Page({
  ...
  onLoad(options) {
    ...
    const sourceZone = !isSharedResult && (options.sourceZone === 'boss' || options.sourceZone === 'play')
      ? options.sourceZone
      : 'play'
    const sourceGroup = !isSharedResult && options.sourceGroup ? decodeURIComponent(options.sourceGroup) : ''
    this.setData({ isSharedResult, sourceZone, sourceGroup })
    if (options.id) {
  ...
  saveImage() {
    ...
    this.reportSaveEvent('original_save_click', 'original')
    this.saveOriginalImage()
  },

  reportSaveEvent(eventName, variant, errorCode = '') {
    report(eventName, {
      template_id: this.data.featureId || '',
      template_version_id: this.data.templateVersionId || '',
      result_id: this.data.id || '',
      generation_id: this.data.taskId || '',
      save_variant: variant,
      zone: this.data.sourceZone || '',
      category_id: this.data.sourceGroup || '',
      source_page: 'generation_result',
  ...
  saveOriginalImage() {
    this.saveImageUrlToAlbum(this.data.resultUrl, '保存成功', 'original')
  },

  saveImageUrlToAlbum(imageUrl, successTitle = '保存成功', variant = 'original') {
    ...
    if (imageUrl.startsWith('cloud://')) {
      wx.cloud.downloadFile({
        fileID: imageUrl,
        success: (res) => this.saveTempFileToAlbum(res.tempFilePath, successTitle, variant),
        fail: () => {
          this.reportSaveEvent(`${variant === 'hd' ? 'hd' : 'original'}_save_failed`, variant, 'download_failed')
    ...
    wx.downloadFile({
      ...
      success: (res) => {
        if (res.statusCode === 200) {
          this.saveTempFileToAlbum(res.tempFilePath, successTitle, variant)
        } else {
          this.reportSaveEvent(`${variant === 'hd' ? 'hd' : 'original'}_save_failed`, variant, 'download_status')
          this.setData({ isSavingImage: false })
          wx.hideLoading()
          wx.showToast({ title: '下载失败', icon: 'none' })
        }
  ...
  saveTempFileToAlbum(filePath, successTitle = '保存成功', variant = 'original') {
    wx.saveImageToPhotosAlbum({
      filePath,
      success: () => {
        this.reportSaveEvent(`${variant === 'hd' ? 'hd' : 'original'}_save_succeeded`, variant)
        this.setData({ isSavingImage: false })
        wx.hideLoading()
        wx.showToast({ title: successTitle, icon: 'success' })
      },
      fail: (err) => {
        const denied = err.errMsg.indexOf('auth deny') > -1 || err.errMsg.indexOf('auth denied') > -1
        this.reportSaveEvent(`${variant === 'hd' ? 'hd' : 'original'}_save_failed`, variant, denied ? 'permission_denied' : 'album_save_failed')
        this.setData({ isSavingImage: false })
        wx.hideLoading()
        if (denied) {
          wx.showModal({
  ...
  showUpscaleError(message, errorCode = 'upscale_failed') {
    this.reportSaveEvent('hd_save_failed', 'hd', errorCode)
  ...
  saveUpscaledFromPanel() {
    if (this.data.isUpscaling || this.data.isSavingImage) return
    this.reportSaveEvent('hd_save_click', 'hd')
    if (this.data.upscaledUrl) {
      this.setData({ showSavePanel: false })
      this.saveImageUrlToAlbum(this.data.upscaledUrl, '高清版已保存', 'hd')
      return
    }
    if (!this.data.id) {
      this.reportSaveEvent('hd_save_failed', 'hd', 'missing_generation_record')
  ...
  async createUpscaleTask() {
    ...
    try {
      ...
      if (!result.success) {
        ...
        this.showUpscaleError(result.error, result.code || 'upscale_create_failed')
        return
      }
      if (result.resultUrl) {
        this.setData({
        ...
        wx.hideLoading()
        this.saveImageUrlToAlbum(result.resultUrl, '高清版已保存', 'hd')
        return
      }
      ...
      if (task.resultUrl) {
        ...
        wx.hideLoading()
        this.saveImageUrlToAlbum(task.resultUrl, '高清版已保存', 'hd')
        return
      }
      if (!task.upscaleTaskId) {
        wx.hideLoading()
        this.setData({ isUpscaling: false })
        this.reportSaveEvent('hd_save_failed', 'hd', 'missing_upscale_task_id')
      ...
      this.showUpscaleError(err && err.message, 'upscale_request_failed')
    }
  },
  ...
  async fetchUpscaleTaskStatus(upscaleTaskId) {
    ...
    try {
      ...
      if (!result.success) {
        wx.hideLoading()
        this.setData({ isUpscaling: false })
        this.showUpscaleError(result.error, result.code || 'upscale_status_failed')
        return
      }
      ...
      if (task.status === 'failed') {
        wx.hideLoading()
        this.setData({ isUpscaling: false })
        this.showUpscaleError(task.errorMessage, task.errorCode || 'upscale_failed')
        return
      }
