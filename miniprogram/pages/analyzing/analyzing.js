...
const { createAnalyzingInterstitial } = require('./interstitial-ad.js')
...
Page({
  ...
  onLoad(options) {
    this.analyzingPageVisible = true
    this.interstitialController = createAnalyzingInterstitial(this)
    this.initSnakeGame()
    const sourceZone = options.sourceZone === 'boss' || options.sourceZone === 'play'
      ? options.sourceZone
      : ''
    const sourceGroup = options.sourceGroup ? decodeURIComponent(options.sourceGroup) : ''
    if (options.taskId) {
      ...
      this.pollTaskStatus(taskId)
      this.preloadResultPackage()
      wx.nextTick(() => this.interstitialController.start(taskId))
      return
    }
  ...
  onHide() {
    this.analyzingPageVisible = false
    this.stopSnakeLoop()
    if (this.interstitialController) this.interstitialController.onPageHide()
  },

  preloadResultPackage() {
    if (this.resultPackagePreloaded || typeof wx.loadSubpackage !== 'function') return
    this.resultPackagePreloaded = true
    wx.loadSubpackage({ name: 'result', fail: () => { this.resultPackagePreloaded = false } })
  },
  ...
  async startAsyncGeneration() {
    try {
      ...
      this.setData({
        ...
        statusText: 'AI 正在绘制中...'
      }, () => wx.nextTick(() => this.interstitialController.start(result.taskId)))
      this.preloadResultPackage()
      this.startGenerationTimer()
  ...
  returnToFeatureDetail(task = {}) {
    ...
    if (featureId) {
      wx.redirectTo({
        url: `/pages/feature/feature?id=${encodeURIComponent(featureId)}${this.data.sourceZone ? `&sourceZone=${this.data.sourceZone}` : ''}${this.data.sourceGroup ? `&sourceGroup=${encodeURIComponent(this.data.sourceGroup)}` : ''}`
      })
      return
    }
    wx.navigateBack()
  ...
  openResultPage() {
    ...
    if (this.data.sourceGroup) {
      params.push(`sourceGroup=${encodeURIComponent(this.data.sourceGroup)}`)
    }
    wx.redirectTo({
  ...
  onUnload() {
    this.analyzingPageVisible = false
    if (this.interstitialController) this.interstitialController.destroy()
    this.stopSnakeLoop()
    this.stopProgress()
    this.stopPolling()
    this.stopGenerationTimer()
